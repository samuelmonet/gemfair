...
def selectdf2(q1,q2,dfm):
	...
	if q1 in cont_cats or q2 in cont_cats:
		if q1 in cont_cats:
			hh,question=q1,q2
		else:
			hh,question=q2,q1
		HH=[i for i in dfm.columns if hh in i and hh[:3]==i[:3]]
		df2=dfm[HH+[question]]
...
def load_data():
	...
	correl_init=pd.read_csv('correlations_init.csv',sep='\t')
	...
	cont_cats=pickle.load( open( "cont_cats.p", "rb" ) )
	dummy_cols=pickle.load( open( "dummy.p", "rb" ) )	
	questions.set_index('Idquest',inplace=True)
	data['year_joined']=data['year_joined'].apply(lambda x: 'Never' if x==0 else str(x))
	return data,correl,questions[[i for i in questions.columns if 'Unnamed' not in i]],codes,continues,cat_cols,dummy_cols,cont_cats,correl_init


#Récup des data avec les variables nouvelles
data,correl,questions,codes,continues,cat_cols,dummy_cols,cont_cats,correl_init=load_data()
...
for i in correl:
	Dico[i]=[]
	for k in correl[i]:
		if k not in correl_init[i].tolist():
			Dico[i].append(k)
	if len(Dico[i])==0:
		del Dico[i]
...
def main():
	...
	#st.write(graphs)
	q1 = st.sidebar.selectbox('Main question:', [None]+[i for i in Dico])
	if q1 != None:
		df=selectdf(q1)
		q2_list=Dico[q1]	
			...
			for q2 in q2_list:
				...
				if q1 in ["longitude","latitude"] or q2 in ["longitude","latitude"]:
					...
					if question in cat_cols:
						...
						st.plotly_chart(px.scatter(df, x="longitude", y="latitude", color=question))
					
								
								
				elif q1 in cat_cols and q2 in cat_cols:
					
					if q1 in cont_cats and q2 in cont_cats:
						dftemp=df[quests1+quests2]
						n=len(quests1)
						col1, col3 = st.columns([1,1])
						
						for i in range(n):
							quest=dftemp[[quests1[i]]+quests2]
							final=pd.DataFrame(columns=[quests1[i],'value',q2])
							
							for j in quests2:
								temp=quest[[quests1[i],j]]
								temp[q2]=temp[j].apply(lambda x:j)
								temp.columns=[quests1[i],'value',q2]
								final=final.append(temp)
							
							final.reset_index(inplace=True)							
							
							
							a=final.groupby([quests1[i],'value',q2]).agg({'index':'count'})
							a.reset_index(inplace=True)
							
							if i%2==0:
								col1.plotly_chart(px.scatter(a, x=q2, y=quests1[i],size='index',\
								hover_data=['index'],color="value"))
							else:
								col3.plotly_chart(px.scatter(a, x=q2, y=quests1[i],size='index',\
								hover_data=['index'],color="value"))
						
						
					
					elif q1 in cont_cats or q2 in cont_cats:
						st.write("à coder")
						st.write(len(q1))
