...
def load_data():
	...
	return data,correl,questions[[i for i in questions.columns if 'Unnamed' not in i]],codes,continues,cat_cols,dummy_cols,cont_cats


#Récup des data avec les variables nouvelles
data,correl,questions,codes,continues,cat_cols,dummy_cols,cont_cats=load_data()
...
def main():
	...
	#st.write(graphs)
	q1 = st.sidebar.selectbox('Main question:', [None]+[i for i in correl.columns if i not in L][70:])
	if q1 != None:
		df=selectdf(q1)
		q2_list=correl[q1]	
						...
						for i in range(n):
							if i%2==0:
								if autre in continues:
									col1.write('coder le double continue')
								else:
									col1.plotly_chart(box(cat_quests[i],autre,cat_quests[i],autre,df3))
							else:
								if autre in continues:
									col3.write('coder le double continue')
								else:
